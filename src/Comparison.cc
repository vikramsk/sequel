...
int *OrderMaker ::getAttributes() { return whichAtts; }
...
int CNF ::GetQueryOrder(OrderMaker &sortOrder, OrderMaker &queryOrder,
    ...
    for (int k = 0; k < sortOrder.numAtts; k++) {
        ...
        for (int i = 0; i < numAnds; i++) {
            ...
            // Also check if the other operand is the same as the attribute we
            // are seeking
            if (orList[i][0].operand1 == Literal &&
                orList[i][0].whichAtt2 == attr) {
                foundAttr = true;
                queryLiteralOrder.whichAtts[queryLiteralOrder.numAtts] =
                    orList[i][0].whichAtt1;

            } else if (orList[i][0].operand2 == Literal &&
                       orList[i][0].whichAtt1 == attr) {
                foundAttr = true;
                queryLiteralOrder.whichAtts[queryLiteralOrder.numAtts] =
                    orList[i][0].whichAtt2;
            }
        ...
        queryLiteralOrder.whichTypes[queryLiteralOrder.numAtts] =
            sortOrder.whichTypes[k];
        queryOrder.numAtts++;
    ...
    for (int whichAnd = 0; 1; whichAnd++, parseTree = parseTree->rightAnd) {
        ...
        for (int whichOr = 0; 1; whichOr++, myOr = myOr->rightOr) {
            ...
            if (myOr->left->left->code == NAME) {
                ...
                // the next thing is to see if we have a string; if so, add it
                // to the literal record that stores all of the comparison
                // values
                ...
                cerr << "You gave me some strange type for an operand that I "
                        "do not recognize!!\n";
                exit(1);
            }
