...
int Record ::ComposeRecord(Schema *mySchema, const char *src) {
    ...
    // the last thing is to set up the pointer to just past the end of the
    // reocrd
...
void Record ::Project(int *attsToKeep, int numAttsToKeep, int numAttsNow) {
    ...
    for (int i = 0; i < numAttsToKeep; i++) {
        ...
        if (attsToKeep[i] == numAttsNow - 1) {
            // in this case, take the length of the record and subtract the
            // start pos
            totSpace += ((int *)bits)[0] - ((int *)bits)[attsToKeep[i] + 1];
        } else {
            // in this case, subtract the start of the next field from the start
            // of this field
...
void Record ::MergeRecords(Record *left, Record *right, int numAttsLeft,
    ...
    for (int i = 0; i < numAttsToKeep; i++) {
        ...
        if (attsToKeep[i] == numAttsNow - 1) {
            ...
            // start pos
            totSpace +=
                ((int *)rec_bits)[0] - ((int *)rec_bits)[attsToKeep[i] + 1];
        } else {
            // in this case, subtract the start of the next field from the start
            // of this field
...
std::string Record ::GetTextVersion(Schema *mySchema) {
    std::stringstream recordBuilder;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (atts[i].myType == Int) {
            ...
            double *myDouble = (double *)&(bits[pointer]);
            recordBuilder << *myDouble;

            // then is a character string
        } else if (atts[i].myType == String) {
            char *myString = (char *)&(bits[pointer]);
            recordBuilder << myString;
        }
