...
int ComparisonEngine ::Compare(Record *left, Record *literal,
    ...
    for (int i = 0; i < myComparison->numAnds; i++) {
        for (int j = 0; j < myComparison->orLens[i]; j++) {
            ...
            // if we made it through all of the comparisons without a hit,
            // return a 0
        ...
        // this ends the for-loop for the OR-list... if we made it this far,
        // then we'll go on to the next disjunction in the overall CNF
        // expression
    }

    // this ends the for-loop for the AND-list... if we made it this far, then
    // we know that we have satisfied the overall CNF expression
    return 1;
}
...
int ComparisonEngine ::Run(Record *left, Record *literal, Comparison *c) {
    ...
    switch (c->attType) {
        ...
        case Int:
            ...
            // and check the operation type in order to actually do the
            // comparison
            switch (c->op) {
                case LessThan:
        ...
        default:
            ...
            // so check the operation type in order to actually do the
            // comparison
