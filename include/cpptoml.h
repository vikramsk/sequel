...
class value : public base {
    ...
    template <class U>
    friend std::shared_ptr<typename value_traits<U>::type> cpptoml::make_value(
        U&& val);
 ...
 */
template <>
inline typename array_of_trait<array>::return_type array::get_array_of<array>()
    const {
...
class table : public base {
   public:
     ...
     * "qualified keys".
     */
    std::shared_ptr<table_array> get_table_array_qualified(
     ...
     */
    template <class T>
    inline typename array_of_trait<T>::return_type get_array_of(
        const std::string& key) const {
   ...
   private:
    ...
    bool resolve_qualified(const std::string& key,
        ...
        if (!p) return table->map_.count(last_key) != 0;
...
class parser {
   ...
   private:
    ...
    void parse_single_table(std::string::iterator& it,
        ...
        while (it != end && *it != ']') {
            auto part =
                parse_key(it, end, [](char c) { return c == '.' || c == ']'; });
    ...
    std::shared_ptr<value<std::string>> parse_string(
    ...
    std::shared_ptr<value<std::string>> parse_multiline_string(
        ...
        auto handle_line = [&](std::string::iterator& it,
                               std::string::iterator& end) {
            if (consuming) {
    ...
    std::shared_ptr<value<double>> parse_float(
        std::string::iterator& it, const std::string::iterator& end) {
        std::string v{it, end};
        v.erase(std::remove(v.begin(), v.end(), '_'), v.end());
        it = end;
    ...
    std::shared_ptr<value<local_time>> parse_time(
        std::string::iterator& it, const std::string::iterator& end) {
